...
static int get_index(const char *ifname, const char *doc)
{
...
format_id_t format = FORMAT_PLAIN;
...
static int cmd_showport(int argc, char *const *argv)
{
    ...
    {
        if(0 > (count = get_bridge_port_list(argv[1], &namelist)))
            return count;
    }
...
out_of_memory_exit:
    ...
    for(i = 1; i <= brcount; ++i)
    {
        ...
        if(0 > (ifcount = get_bridge_port_list(argv[i], &namelist)))
        {
ifaces_error_exit:
            for(i -= 2; i >= 0; --i)
                free(ifaces_lists[i]);
